...
void loop_vector_w_index_ref(std::vector<int> &nums)//here we are passing by reference
{
	for (int i = 0; i < nums.size(); i++)
	{
		std::cout << nums[i] << "\n";
...
void loop_vector_w_index_ref_const(const std::vector<int> &nums)//here we are passing a const
{
	for (int i = 0; i < nums.size(); i++)
	{
		std::cout << nums[i] << "\n";
